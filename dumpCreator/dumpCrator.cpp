...
)
{
    ...
    if (hProc == nullptr)
    {
        return 0; //cant't open process
    }

    auto hDbgHelp = LoadLibraryA("dbghelp");
    if (hDbgHelp == nullptr)
        return 0; // can't load dbghelp
    auto pMiniDumpWriteDump = (decltype(&MiniDumpWriteDump))GetProcAddress(hDbgHelp, "MiniDumpWriteDump");
    if (pMiniDumpWriteDump == nullptr)
        return 0; // cant't getprocaddress 

    unsigned int result = 0; // assume the worst
    ...
    ))
    {
        ...
        {
            result = 0; // ERROR_CANCELED
        }
    }
    else
    {
        result = 1; // ERROR_SUCSESS
    }
